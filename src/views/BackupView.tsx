...
import { Database, Download, Upload, CheckCircle2, AlertTriangle, ShieldCheck, FileJson } from 'lucide-react';
import { exportDatabaseToJSON, importDatabaseFromJSON } from '../utils/backup';
    ...
    <div className="space-y-6 max-w-4xl mx-auto pb-12">
      ...
      <div className="space-y-1">
        ...
        <p className="text-xs text-zinc-400">
          Exporta todos tus dispositivos y fotos a un archivo JSON local o restaura una copia anterior en cualquier momento.
        </p>
      </div>
        ...
        >
          ...
          <div>
            <div className="font-bold text-sm">
              {importResult.success ? '¡RESTAURACIÓN COMPLETADA CON ÉXITO!' : 'ERROR AL RESTAURAR'}
            </div>
            <p className="mt-0.5">
              {importResult.success
                ? `Se importaron ${importResult.count} dispositivos correctamente a tu base de datos local.`
                : importResult.error}
            </p>
          </div>
        </div>
        ...
        <div className="bg-zinc-900 border border-zinc-800/80 rounded-2xl p-6 space-y-4 flex flex-col justify-between">
          <div className="space-y-3">
            ...
            <p className="text-xs text-zinc-400 leading-relaxed">
              El archivo descargado contiene los metadatos completos, identificadores, fechas, precios y todas las fotografías comprimidas de tus <span className="text-white font-semibold">{deviceCount} dispositivos</span>.
            </p>
          </div>
        ...
        <div className="bg-zinc-900 border border-zinc-800/80 rounded-2xl p-6 space-y-4 flex flex-col justify-between">
          <div className="space-y-3">
            ...
            <div className="space-y-2 text-xs">
              <label className="block text-zinc-400 font-medium">Modo de importación:</label>
          ...
          <button
            ...
            className="w-full py-3 px-4 bg-zinc-800 hover:bg-zinc-700 border border-zinc-700 text-white text-xs font-semibold rounded-xl transition-all flex items-center justify-center space-x-2 cursor-pointer"
          >
