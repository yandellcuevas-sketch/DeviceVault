...
import { Settings, Trash2, Database, Shield, AlertTriangle } from 'lucide-react';
import { db } from '../db';
...
export const SettingsView: React.FC<SettingsViewProps> = ({ deviceCount, onRefresh }) => {
  ...
  const handleClearDatabase = async () => {
    ...
    setClearedMsg(true);
    onRefresh();
    setTimeout(() => setClearedMsg(false), 4000);
  };
    ...
    <div className="space-y-6 max-w-4xl mx-auto pb-12">
      <div className="space-y-1">
        <div className="flex items-center space-x-2">
          ...
          <h2 className="text-xl font-bold text-white tracking-tight">Configuración y Almacenamiento</h2>
        </div>
        <p className="text-xs text-zinc-400">
          Información sobre el almacenamiento local de DeviceVault y gestión de datos.
        </p>
      </div>
      ...
      <div className="bg-zinc-900 border border-zinc-800/80 rounded-2xl p-6 space-y-4">
        <h3 className="text-sm font-bold text-white uppercase tracking-wider border-b border-zinc-800 pb-3 flex items-center gap-2">
          <Database className="w-4 h-4 text-blue-400" /> Estado de la Bóveda Local
        </h3>
      ...
      {/* Zona Peligrosa: Limpiar base de datos */}
      <div className="bg-zinc-900 border border-red-900/40 rounded-2xl p-6 space-y-4">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h3 className="text-sm font-bold text-white">Zona de Peligro: Restablecer Bóveda</h3>
            <p className="text-xs text-zinc-400">Elimina permanentemente todos los registros y fotografías locales.</p>
          </div>
        </div>

        <p className="text-xs text-zinc-400 leading-relaxed">
          Esta acción vaciará la base de datos IndexedDB local. Recuerda exportar un respaldo en formato JSON antes si deseas conservar tus datos.
        </p>
      ...
      <ConfirmDialog
        ...
        title="¿Restablecer Bóveda Completa?"
        message={`¿Estás seguro de que deseas eliminar TODOS los ${deviceCount} dispositivos y sus fotos de este navegador? Te recomendamos exportar un respaldo primero.`}
